...
var Grid = class {
  constructor(container, data, columns, options) {
    ...
    this._paneHeaderL = H(
      ...
      H(
        "div",
        { class: "slick-header slick-header-left" + uisd, style: !options.showColumnHeader && "display: none" },
    ...
    this._paneHeaderR = H(
      ...
      H(
        "div",
        { class: "slick-header slick-header-right" + uisd, style: !options.showColumnHeader && "display: none" },
        this._headerColsR = H("div", { class: "slick-header-columns slick-header-columns-right", style: this._rtlS + ":-1000px" })
      )
    );
    var headerRowL = H(
      "div",
      { class: "slick-headerrow" + uisd, style: !options.showHeaderRow && "display: none" },
    ...
    var topPanelLS = H(
      "div",
      { class: "slick-top-panel-scroller" + uisd, style: !options.showTopPanel && "display: none" },
      this._topPanelL = H("div", { class: "slick-top-panel", style: "width: 10000px" })
    );
    this._viewportTopL = H(
      "div",
      { class: "slick-viewport slick-viewport-top slick-viewport-left", tabIndex: "0", hideFocus: "" },
      this._canvasTopL = H("div", { class: "grid-canvas grid-canvas-top grid-canvas-left", tabIndex: "0", hideFocus: "" })
    );
    var footerRowL = H(
      "div",
      { class: "slick-footerrow" + uisd, style: !options.showFooterRow && "display: none" },
    ...
    this._paneTopL = H(
      "div",
      { class: "slick-pane slick-pane-top slick-pane-left", tabIndex: "0" },
      headerRowL,
      topPanelLS,
      this._viewportTopL,
      footerRowL
    ...
    this._viewportTopR = H(
      "div",
      { class: "slick-viewport slick-viewport-top slick-viewport-right", tabIndex: "0", hideFocus: "" },
      this._canvasTopR = H("div", { class: "grid-canvas grid-canvas-top grid-canvas-right", tabIndex: "0", hideFocus: "" })
    );
    var footerRowR = H(
      "div",
      { class: "slick-footer-row" + uisd, style: !options.showFooterRow && "display: none" },
    ...
    this._paneTopR = H(
      "div",
      { class: "slick-pane slick-pane-top slick-pane-right", tabIndex: "0" },
      headerRowR,
      topPanelRS,
      this._viewportTopR,
      footerRowR
    ...
    this._paneBottomL = H(
      "div",
      { class: "slick-pane slick-pane-bottom slick-pane-left", tabIndex: "0" },
      this._viewportBottomL = H(
        "div",
        { class: "slick-viewport slick-viewport-bottom slick-viewport-left", tabIndex: "0", hideFocus: "" },
        this._canvasBottomL = H("div", { class: "grid-canvas grid-canvas-bottom grid-canvas-left", tabIndex: "0", hideFocus: "" })
      )
    );
    this._paneBottomR = H(
      "div",
      { class: "slick-pane slick-pane-bottom slick-pane-right", tabIndex: "0" },
    ...
    this._container.append(
      ...
      this._paneTopR,
      this._paneBottomL,
      this._focusSink2 = this._focusSink1.cloneNode()
    );
  ...
  setupColumnReorder() {
    ...
    $([this._headerColsL, this._headerColsR]).sortable({
      ...
      stop: (e) => {
        ...
        ;
        var reorderedCols = sortToDesiredOrderAndKeepRest(
          this._initCols,
          this._headerColsL.sortable("toArray").map((x) => x.substring(this._uid.length))
        );
        reorderedCols = sortToDesiredOrderAndKeepRest(
          reorderedCols,
          this._headerColsR.sortable("toArray").map((x) => x.substring(this._uid.length))
        );
  ...
  setupColumnResize() {
    if (typeof $ === "undefined" || !$.fn || !$.fn.drag)
      return;
  ...
  measureCellPaddingAndBorder() {
    ...
    el.remove();
    var r = this._canvasTopL.appendChild(H(
      "div",
      { class: "slick-row" },
  ...
  applyColumnHeaderWidths() {
    ...
    for (var i = 0, ii = headersL.length + headersR.length; i < ii && i < colsL; i++) {
      h = (frozenCols && i >= frozenCols ? headersR : headersL).item(i);
  ...
  getViewportHeight() {
    ...
    if (this._options.autoHeight) {
      ...
      var columnNamesH = this._options.showColumnHeader ? parseFloat(getComputedStyle(this._headerColsL.parentElement).height) + this.getVBoxDelta(this._headerColsL.parentElement) : 0;
      ...
      this._viewportH = parseFloat(style.height) - parseFloat(style.paddingTop) - parseFloat(style.paddingBottom) - columnNamesH - this._topPanelH - this._headerRowH - this._footerRowH - this._groupingPanelH;
    }
